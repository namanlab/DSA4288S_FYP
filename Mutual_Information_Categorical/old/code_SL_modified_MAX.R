...
gen_number <- function(x) {
  ...
  # Create table copies for delta1 and delta2
  table1 <- table
  table2 <- table
  ...
  # Calculate mutual information for both tables
  mutinf1 <- MutInf(table1)
  mutinf2 <- MutInf(table2)
  
  # Choose the table with the larger mutual information
  if (mutinf1 > mutinf2) {
    return(table1)
  } else {
    return(table2)
...
df <- data.frame(
  ...
  y = sample(LETTERS[6:9], 1000, replace = TRUE)
)
