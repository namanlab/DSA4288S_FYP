...
adjust_matrices <- function(matrices, max_n = 1000, temp = 10) {
  ...
  for (n in 1:max_n) {
    ...
    t <- temp / (n + 1)
    metropolis <- ifelse(curr_eval_sum >= 0, exp((curr_eval_sum - new_eval_sum) / t),
                         exp((new_eval_sum - curr_eval_sum)/t))
...
result <- adjust_matrices(matrices, max_n = 1000)
...
# Run the generalized log-odds adjustment on matrices
result <- adjust_matrices(matrices, max_n = 4000)
